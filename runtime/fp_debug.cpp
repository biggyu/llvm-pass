...
template <typename T>
double incorrect_bits_bitwise(T x, T dx) {
...
static void check_error_impl(T x, double dx, int site_id, int metric, uint64_t &total_checks, std::unordered_map<int, SiteStats> &sites) {
    ...
    bool finite_metric = errcls != ErrorClass::XZero && errcls != ErrorClass::NaN && errcls != ErrorClass::Inf && std::isfinite(bits) && std::isfinite(relerr);
    if (!finite_metric) {
        S.warn_4++;
        S.warn_8++;
        S.warn_16++;
