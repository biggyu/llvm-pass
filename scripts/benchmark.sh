#!/usr/bin/env bash
# sh scripts/run_benchmark.sh ShadowMem shadowmem benchmarks/fpbench/foo.c benchmarks/driver/foo_main.c
...
if [ $# -ne 3 ]; then
    echo "Usage: $0 <PASS> <PLUGIN> <BENCH_SRC>"
    exit 1
fi
...
$LLVM_CLANG -c "$DRIVER_SRC" -o "$OUTDIR/driver.o"
